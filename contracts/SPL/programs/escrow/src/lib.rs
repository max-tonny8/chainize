...
pub mod escrow {
    ...
    /// Reset the authority from **Program Derived Address**.
...
pub struct EscrowAccount {
    /** `PubKey` of person who started a new `EscrowAccount` used to authorize the actions properly */
    pub initializer_key: Pubkey,
    /// `PubKey` to record the deposit account of initialzer
    pub initializer_deposit_token_account: Pubkey,
    /// `PubKey` to record the receiving account of initializer
    pub initializer_receive_token_account: Pubkey,
    /// `u64` amount to record how much token should the initializer transfer to taker
    pub initializer_amount: u64,
    /// `u64` amount to record how much token should the initializer receive from the taker
